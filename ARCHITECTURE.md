...
康奈尔视图**只做排版**。把一篇笔记按 `##` 拆成线索 / 正文 / 总结三种卡片，按康奈尔的样子摆好，
仅此而已。编辑是**现成功能**：双击任意位置切成原有的 `KeywordNote` 卡片，由它自带的整篇实时编辑器接手。

因此：

- ❌ 不要为康奈尔另写编辑器宿主、另调 `spawnLeafView`。
- ❌ 不要做「双击线索就把光标定位到 `##` 行」这类分区定位。既然编辑态是整篇卡片，
  就没有分区定位这个问题，加了只会污染 `KeywordNote` 的接口。
- ❌ 不要用 CodeMirror 装饰把康奈尔布局叠在实时编辑器上（试过块级隐藏 `##` 行 + widget
  把线索搬进左列：线索无法正常编辑、方向键跳过标题行，编辑手感全毁）。
- ✅ `KeywordNote.svelte` 必须与关键词视图**完全一致，一行都不改**。
  `CornellNote.svelte` 只是在编辑态原样渲染它，外层不加边框、内边距、背景。
...
```markdown
#tag 或任意引言        ← 第一个 ## 之前的内容，归入正文卡开头（绝不隐藏内容）

## 线索1              ← 二级标题 = 线索，聚合进线索卡
我是线索1正文          ← 标题下的内容 = 正文，聚合进正文卡

## 线索2
我是线索2正文

## 随便叫什么          ← 最后一个 ## 直接就是总结，不看标题文字
我是总结              ← 变成横贯全宽的总结卡
```

判定规则就一条：**最后一个 `##` 分区 = 总结**。不要加「标题必须叫 总结/Summary」之类的
关键词匹配，加过一次，用户标题写「最后一个标题识别为总结」就没命中。
唯一的例外是全文只有一个 `##`，此时不拆，否则正文卡会被掏空。
...
- **所有线索聚合成一篇**（线索卡），**所有正文聚合成一篇**（正文卡），总结单独一篇。
- ❌ 不要一条线索一张卡、按行切格子。那样会渲染成表格的样子，明确被否掉过两次。
  线索列和正文列各自是**一个整体**，不做逐行对齐。
- 布局就是一个两列 grid（`var(--kw-cornell-cue-width) minmax(0, 1fr)`），只有两个子节点。
- 正文卡内容 = `topic.body` + 各段 `body` 用 `\n\n` 拼成一个 markdown 串，一次性渲染。
  线索卡内容 = 各段 `cue` 逐行列出的纯文本。

### 实现要点

- `utils/cornellParser.ts` 纯解析：返回 `topic` / `rows` / `summary`。
- 只读渲染容器要带 `markdown-rendered` class，否则 Obsidian 排版样式不生效（字号行距全乱）。
- `MarkdownRenderer.render` 传组件自己 `new Component()` 并 `load()` 的实例，
  `onDestroy` 里 `unload()`；传 `plugin` 会让子组件拖到插件卸载才释放。
- 单击只处理链接跳转与选中，双击才进编辑；`Esc` / 焦点离开 / 右上角按钮回到排版。
- `metadataCache.on("changed")` 触发重新解析，编辑态期间跳过。
- 窄于 560px（ResizeObserver 量卡片实际宽度，不是窗口宽度）时线索卡堆到正文卡上方。
- 工具栏三个开关（线索 / 正文 / 总结）只控制渲染哪几种卡片，状态存在
  `settings.cornellShowCue/Body/Summary`，线索列宽度存 `settings.cornellCueWidth`。
